...
type WebSocketDonChannelConfig = {
  /**
   * WebSocketサーバーのオプション
   */
...
export class WebSocketDonChannel implements DonChannel {
  ...
  readonly config: WebSocketDonChannelConfig;
  private static readonly defaultConfig: WebSocketDonChannelConfig = {
    wssConfig: {},
  };
  ...
  constructor(config: WebSocketDonChannelConfig) {
    this.config = { ...WebSocketDonChannel.defaultConfig, ...config };
    logger.debug('WebSocketDonChannel initialized with config', { data: this.config });
