...
router.post(
  '/price',
  typedAsyncWrapper<'/dons/price', 'post'>(async (req, res) => {
...
router.post(
  '/status/',
  typedAsyncWrapper<'/dons/status/', 'post'>(async (req, res, next) => {
...
router.put(
  '/:id',
  typedAsyncWrapper<'/dons/{id}', 'put'>(async (req, res, next) => {
    const id = req.params.id;
    const status = req.body.status;
