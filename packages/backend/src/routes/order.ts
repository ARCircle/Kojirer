...
router.post(
  '/',
  typedAsyncWrapper<'/order', 'post'>(async (req, res, next) => {
...
router.post(
  '/price',
  typedAsyncWrapper<'/order/price', 'post'>(async (req, res) => {
    const dons = req.body.dons;

    if (!dons) throw ApiError.invalidParams();
    ...
    const prices = await Promise.all(
      dons.map(async (don) => {
        ...
        const discount = isFollowed
          ? -1 *
            toppingsPrices.reduce((lowestPrice, topping) => {
              const price = topping.topping_prices[0].price;
...
router.post(
  '/status',
  typedAsyncWrapper<'/order/status', 'post'>(async (req, res, next) => {
...
router.put(
  '/status',
  typedAsyncWrapper<'/order/status', 'put'>(async (req, res, next) => {
