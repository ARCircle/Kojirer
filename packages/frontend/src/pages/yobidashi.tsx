  ...
  return (
    <Box h='100%' w='100%' pos='relative' borderColor='gray' border='4px' borderRadius='16px' overflow='hidden'>
      ...
      <Flex flexDirection='row' px='10px' h='100%' pos='absolute' w='100%'>
        <Center w='50%' fontWeight='bold' fontSize={responsiveFontSize} color='black' pr='2px'>
          {order.id}
        </Center>
        <Center w='2%'>
        ...
        <Center
          ...
          fontSize={responsiveFontSize}
          fontWeight='bold'
        >{`${order.dons.filter((don) => don.status === 2).length}/${order.dons.length}`}</Center>
      </Flex>
    </Box>
  );
};
...
const Yobidashi: React.FC = () => {
  ...
  const TableLabel: React.FC<TableLabelProp> = ({ name }: TableLabelProp) => {
    return (
      <Heading textAlign='center' p='2px' w='fit-content' mx='auto' fontSize='5xl' fontWeight='bold'>
        {name}
      </Heading>
    );
  };
